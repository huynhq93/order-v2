<template>
  <div class="order-code-manager">
    <el-card class="w-full">
      <template #header>
        <div class="header-container">
          <div class="header-row">
            <h3 class="title">Nhập mã đặt hàng</h3>
          ...
          <!-- Single Order Code Input -->
          <div class="order-code-input-section">
            <div class="input-row">
              <el-input
                v-model="singleOrderCode"
                placeholder="Nhập mã đặt hàng (VD: DH123)"
                style="width: 300px;"
                clearable
              >
                <template #prefix>
      ...
      <div class="content-container">
        ...
        <div class="action-row" v-if="selectedOrders.length > 0">
          ...
          <div class="action-buttons">
            ...
            <el-button 
              type="primary" 
              @click="applyOrderCode"
              :disabled="!singleOrderCode.trim()"
            >
              Áp dụng mã đặt hàng
            </el-button>
          </div>
...
const singleOrderCode = ref('')
const selectedOrders = ref<Order[]>([])

// Computed
const pendingOrders = computed(() => {
  return store.orders.filter(order => order.status === 'NHẬN ĐƠN')
})
...
const applyOrderCode = () => {
  if (selectedOrders.value.length === 0) {
    ElMessage.warning('Vui lòng chọn ít nhất một đơn hàng')
    return
  }
  
  if (!singleOrderCode.value.trim()) {
    ElMessage.warning('Vui lòng nhập mã đặt hàng')
    return
  }
...
const confirmBatchUpdate = async () => {
  updating.value = true
  try {
    const orderCode = singleOrderCode.value.trim().toUpperCase()
    
    // Update each selected order
    for (const order of selectedOrders.value) {
      await store.updateOrderWithCode(
        order.rowIndex,
        orderCode,
        'ĐANG VẬN CHUYỂN',
        props.selectedDate,
    ...
    ElMessage.success(`Đã cập nhật ${selectedOrders.value.length} đơn hàng với mã ${orderCode}`)
    
    // Clear selection and refresh
    clearSelection()
    singleOrderCode.value = ''
